...
* **Evidence**: This maps directly to the counter-curvature principle where energetic supply scales proportionally to $L^2$ and structural cost to $L^4$, leading to systemic exhaustion (Krishnan S et al., 2025).

### 12. The Manganese-Dependent Matrix Axis
* **Path**: `SLC39A8` $\rightarrow$ `Manganese Transport` $\rightarrow$ `GAG Synthesis` $\rightarrow$ `ECM Assembly`
* **Mechanism**: SLC39A8 is a primary manganese transporter. Manganese is an essential cofactor for glycosyltransferases, which synthesize glycosaminoglycans (GAGs) critical for extracellular matrix (ECM) assembly and proteoglycan maturation in cartilage.
* **Role**: Sets the foundational material quality of the spine. Insufficient manganese transport impairs GAG synthesis, leading to compromised ECM integrity and structurally weak cartilaginous tissues (e.g., intervertebral discs and growth plates).
* **Evidence**: Haller et al. (2018) established a direct link between pleiotropic missense variants in *SLC39A8* and severe adolescent idiopathic scoliosis, mediated by reduced systemic manganese levels and defective glycosylation.
> **Hypothesis**: Within the counter-curvature framework, the rapid $L^3$ volume expansion during adolescent growth critically taxes the manganese supply chain. A subtle deficit in SLC39A8 transport capacity creates a metabolic bottleneck for ECM maturation, lowering tissue Stiffness ($EI$) below the critical threshold needed to resist escalating mechanical buckling moments, thereby precipitating scoliotic collapse.
