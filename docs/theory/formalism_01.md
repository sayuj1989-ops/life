...
### 2.40. The Asymmetric Gravitational Bending Constant ($\Gamma_{asym}$)

We formalize the relationship between gravity-induced coronal plane joint moments and asymmetric vertebral growth, extending the Hueter-Volkmann principle. We define $\Gamma_{asym}$ as the sensitivity of the differential coronal growth strain rate ($\Delta \dot{\varepsilon}_{coronal}$) to the local gravity-induced coronal plane joint moment ($M_{coronal}$).

$$ \Delta \dot{\varepsilon}_{coronal} = \Gamma_{asym} \cdot M_{coronal} $$

*   **Symbols**:
    *   $\Delta \dot{\varepsilon}_{coronal}$: Differential growth strain rate across the coronal plane of the vertebra (convex minus concave) $[T^{-1}]$.
    *   $M_{coronal}$: Gravity-induced coronal plane joint moment $[M L^2 T^{-2}]$.
    *   $\Gamma_{asym}$: Asymmetric Gravitational Bending Constant $[M^{-1} L^{-2} T]$.
*   **Dimensions**:
    *   $[\Gamma_{asym}] = [\Delta \dot{\varepsilon}_{coronal}] / [M_{coronal}] = [T^{-1}] / [M L^2 T^{-2}] = [M^{-1} L^{-2} T]$.
*   **Physical Interpretation**: A higher $\Gamma_{asym}$ indicates that the growth plate is highly sensitive to asymmetric load generated by gravitational moments, driving rapid wedging and curve progression.
*   **Measurable Proxy**: The ratio of the longitudinal growth asymmetry (measured via serial radiographs, $mm/year$) to the calculated gravity-induced coronal plane joint moment (derived from 3D reconstructions and body mass distribution).

#### Falsifiable Tests

*   **Test 1: The Coronal Moment Offloading Rescue**
    *   **Data Needed**: Measurement of vertebral wedging progression in a scoliotic animal model or patient cohort before and after specific biomechanical offloading of the coronal moment (e.g., via targeted asymmetric bracing or asymmetric muscle stimulation), tracking $M_{coronal}$ via modeling.
    *   **Refutation**: If the differential growth rate $\Delta \dot{\varepsilon}_{coronal}$ does not decrease proportionally when $M_{coronal}$ is mechanically reduced to zero, the hypothesis that gravitational moments actively drive the growth asymmetry is falsified. (Reference: Keenan et al., 2015).
*   **Test 2: The Symmetric Overload Independence**
    *   **Data Needed**: Vertebral growth tracking in an induced scoliosis model subjected to pure, symmetric axial overload (increasing total compressive force without altering the coronal moment).
    *   **Refutation**: If pure symmetric overload significantly accelerates the *differential* growth wedging ($\Delta \dot{\varepsilon}_{coronal}$) without a corresponding increase in the coronal moment arm, the coupling relationship is not specific to the asymmetric bending moment. (Reference: Stokes, 2006).
