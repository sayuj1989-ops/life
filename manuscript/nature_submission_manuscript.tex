...
We validated this scaling limit across species by computing the Bio-Gravitational Number ($Bg$) for 9 vertebrates using disc mechanics data\cite{oconnell2007}. While quadrupeds operate in a transverse loading regime where low stiffness leads only to benign sagging, humans occupy a unique intersection of low stiffness ($Bg \approx 0.017$) and axial loading, creating a zone of catastrophic buckling instability. This ``Allometric Trap'' forces humans to rely almost entirely on metabolic expenditure to prevent collapse. This explains why scoliosis is uniquely human: only we have evolved a spinal geometry that demands more energy than passive structure can provide.
...
\textbf{Cross-Species Scaling.} We aggregated mass ($M$), spinal length ($L$), and stiffness ($EI$) data for 9 species from O'Connell et al. (2007). The Bio-Gravitational Number was computed as $Bg = EI / (M g L^2)$. A simplified ``Thermostatic Column'' toy model was implemented to verify that metabolic flux $J$ can induce symmetry breaking when demand ($L^5$) outpaces supply capacity ($L^2$).
...
\bibitem{oconnell2007} O'Connell, G.D., Vresilovic, E.J. \& Elliott, D.M. Comparison of animals used in disc research to human lumbar disc geometry. \textit{Spine} \textbf{32}, 328–333 (2007).
