% ORPHANED: not input by main.tex -- see AUDIT_LEDGER.md gotcha 1; edits here never reach the PDF
...
In contrast, the biological machinery supplying this energy—specifically the vascular cross-section and mitochondrial surface area—scales allometrically as $L^2$ or at best $L^{2.25}$ according to Kleiber's Law~\cite{west1997general, rolfe1997cellular}. This constraint is compounded by the Hueter-Volkmann law, which we take as the load-bearing \emph{cementing} step of the recovery ratchet: growth rate is modulated by mechanical stress, so a sustained un-recovered deviation is progressively converted into permanent set ($d\kappa_p/dt \propto k_g\,\kappa_e$; Theory, Eq.~\ref{eq:recovery_ratchet})~\cite{villemure2009growth, stokes2007growth}. This divergence creates a critical \textbf{Energy Deficit Window} during the rapid adolescent growth spurt.
...
\end{equation}
``Spinal Jetlag'' is defined as a stable phase shift $\Delta \phi = \phi_L - \phi_R \neq 0$. This creates a differential growth rate that integrates into a permanent wedge deformity, potentially initiated by organ rotation asymmetry.
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{PPARGC1A (PGC-1$\alpha$):} The master regulator of mitochondrial biogenesis. Its intrinsically disordered nature makes it a sensitive rheostat for cellular energy status. Downregulation in paraspinal muscles has been proposed to precede deformity (citation removed pending a verified source).
        \item \textbf{NAD+ / SIRT1:} The fuel gauge of the cell. Depletion of NAD+ pools during rapid growth inhibits SIRT1-mediated deacetylation of clock proteins, linking metabolism to circadian desynchrony. Melatonin signaling deficiency further compromises this clock synchronization.
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Vimentin (VIM):} An intermediate filament acting as a gravitational strain gauge. Its extreme anisotropy (7.5) makes it the ``canary in the coal mine'' for energy deficits~\cite{hu2019vimentin}.
    \end{itemize}
