...
\begin{figure}[h!]
    ...
    \caption{\textbf{The Energy Deficit Window as a Driver of Vector-Scalar Mismatch.} Thermodynamic cost of countercurvature ($P_{\mathrm{counter}}$, red solid) versus proprioceptive supply capacity ($S_{\mathrm{proprio}}$, blue/green dashed) as a function of spinal length $L$. The intersection at $L_{\mathrm{crit}} \approx 0.35$ m defines a developmental window ($L < L_{\mathrm{crit}}$) where metabolic demand exceeds supply. In this deficit zone, the system must rely on high-gain feedback to maintain alignment. This high-gain state amplifies the `Vector-Scalar Mismatch' (Fig.~\ref{fig:scoliosis_emergence}), where scalar noise from non-directional sensors (e.g., Piezo1) is misinterpreted as directional error, driving scoliotic buckling.}
