...
\begin{figure}[h!]
    ...
    \caption{\textbf{The Biological Counter-Curvature Mechanism as Active Inference.} (A) \textbf{The Generative Model}: Discrete HOX gene expression domains (colored bars) encode a digital ``prior'' that is smoothed into a continuous \textbf{Reference Metric} $\bar{g}$ (blue curve). This serves as the Top-Down prediction of the body's geometry in gravity. (B) \textbf{Sensory Evidence and Prediction Error}: Gravity acts as the ``Interrogator,'' creating a passive sag (solid line, Actual Metric $g$). Mechanosensors (Vector: Piezo2/Lamin A/C; Scalar: Piezo1) measure the divergence between the prediction ($\bar{g}$) and reality ($g$), computing the \textbf{Free Energy} or Error Signal $\mathcal{E} \approx (g - \bar{g})^2$. (C) \textbf{Active Minimization}: The system minimizes this Free Energy via \textbf{Action} (generating counter-curvature moments) and \textbf{Perception} (adjusting sensory gain). In microgravity, the loss of Vector input (alignment) creates a \textbf{Vector-Scalar Mismatch}, leading to gain-driven hallucinations and spinal drift (Spinal Jetlag).}
...
\begin{figure}[h!]
    ...
    (D) Microgravity adaptation ($g \to 0$). The system relaxes towards the information reference state. The reduced mechanical strain $\gEff$ shown here corresponds to the unloading predicted to trigger the ``Nuclear Stiffness Gauge'' downregulation (Active Inference failure), leading to the adipogenic switch.}
...
\begin{figure}[h!]
    ...
    \caption{\textbf{The Energy Deficit Window.} Thermodynamic cost of countercurvature ($P_{\mathrm{counter}}$, red solid) versus proprioceptive supply capacity ($S_{\mathrm{proprio}}$, blue/green dashed) as a function of spinal length $L$. The cost $P_{\mathrm{counter}}$ scales invariantly with length ($L^0$) in the linear regime, while the supply capacity grows sublinearly ($\alpha=0.5$). The intersection at $L_{\mathrm{crit}} \approx 0.35$ m defines an early developmental window ($L < L_{\mathrm{crit}}$) where metabolic demand exceeds supply, necessitating high proprioceptive gain. Beyond this threshold, the mature supply capacity stabilizes the system, though geometric instabilities (Fig.~\ref{fig:scoliosis_emergence}) may still arise from patterning errors.}
