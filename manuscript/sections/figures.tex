...
\begin{figure}[h!]
    ...
    (C) Geodesic deviation $\widehat{D}_{\mathrm{geo}}$ scales with coupling strength; stability requires $\mathcal{B}_g$ to exceed a critical threshold dependent on the Trace of the Anisotropy Tensor ($\text{Tr}(\mathbf{\Lambda})$).
    (D) In microgravity ($g \to 0$), the spine relaxes but retains its "reference shape" due to internal stiffness, matching predictions of spinal elongation (active stiffness modulation) rather than complete straightening.}
