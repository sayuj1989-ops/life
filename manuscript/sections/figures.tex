...
\begin{figure}[h!]
    ...
    \caption{\textbf{Counter-Curvature as Active Morphogenesis and Thermodynamic Maintenance.} (A) \textbf{Passive Gravitational Prior}: In the absence of active correction, gravity dictates a low-energy, monotonic C-shaped sag (blue curve). (B) \textbf{Active Counter-Curvature Posterior}: Biological growth, governed by Information-Elasticity Coupling (IEC) and patterned by HOX genes, generates an S-shaped ``Counter-Curvature'' (orange curve) that actively opposes the gravitational metric. This complex geometry is a thermodynamic standing wave maintained by the continuous expenditure of metabolic work ($P_{\mathrm{counter}}$). (C) \textbf{The Energy Deficit Window}: During rapid adolescent growth, the thermodynamic demand ($P_{\mathrm{counter}}$) required to fund high-anisotropy sensors (e.g., PIEZO2, Lamin A/C) outpaces the diffusion-limited nutrient supply capacity ($S_{\mathrm{proprio}}$). This structural insolvency triggers a ``Metabolic Buckling'' event, collapsing the high-information counter-curvature into the passive, gravity-dominated scoliotic mode.}
