...
\begin{figure}[h!]
    ...
    \caption{\textbf{Thermodynamic Collapse: The Energy Deficit Window.} The metabolic power $P_{\mathrm{counter}}$ (red) required to maintain the Counter-Curvature active standing wave scales as $\sim L^2$. In contrast, the proprioceptive supply capacity (blue), governed by sublinear maturation, scales as $L^\alpha, \alpha \in \{0.5, 1.0\}$. The intersection at $L_{\mathrm{crit}} \approx 0.343$~m marks the onset of the \textbf{Energy Deficit Window} ($L > L_{\mathrm{crit}}$), where the metabolic deficit leaves the spine thermodynamically unable to maintain active countercurvature. Structural analysis of \textbf{Thermodynamic Cost Proteins} reveals the molecular basis of this deficit: high-anisotropy sensors like Piezo2 and Lamin A/C (demand-side) are structurally expensive to maintain~\cite{lynch2015bioenergetic}, while compact supply-side enzymes (PLOD1, MMPs) are cheaper but rate-limited. In the deficit regime, the system cannot afford the high-fidelity state, triggering a \textbf{Convective Shutdown}~\cite{sato2025convective} and a collapse into the low-energy scoliotic mode.}
