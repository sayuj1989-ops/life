...
\begin{figure}[h!]
    ...
    Panel A shows curvature profiles for passive (gravity-only) and IEC-coupled configurations, demonstrating the emergence of S-shaped equilibria. Panel B displays the countercurvature metric $g_{\mathrm{eff}}(s)$, highlighting regions of high counter-curvature demand where tissue anisotropy $\mathbf{\Lambda}$ must exceed the critical threshold $\Lambda_{crit}$ to prevent buckling (Test D). Panel C shows how geodesic deviation $\widehat{D}_{\mathrm{geo}}$ scales with coupling strength $\chi_{\kappa}$, revealing a phase transition at critical coupling. Panel D demonstrates the persistence of counter-curvature in microgravity ($g \to 0$), explaining the stability of spinal curves in orbit (Test C).}
