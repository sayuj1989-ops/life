...
\begin{figure}[h!]
    ...
    \caption{\textbf{Thermodynamic Collapse: The Energy Deficit Window.} The metabolic power $P_{\mathrm{counter}}$ (red) scales as $L^2$ under isometric growth assumptions, reflecting the increasing cost of maintaining counter-curvature against gravity. In contrast, the proprioceptive supply capacity (blue, $S_{\mathrm{proprio}} \sim L^{0.5}$) lags behind. The intersection at $L_{\mathrm{crit}} \approx 0.35$ m marks the onset of the \textbf{Energy Deficit Window} ($L > L_{\mathrm{crit}}$), resulting in a 45.8\% metabolic deficit at $L=0.45$ m. Structural analysis of \textbf{Thermodynamic Cost Proteins} (Table 2) reveals the molecular basis of this deficit: high-anisotropy sensors like Piezo2 and Lamin A/C (demand-side) are structurally expensive to maintain~\cite{lynch2015bioenergetic}, while compact supply-side enzymes (PLOD1, MMPs) are cheaper but rate-limited. In the deficit regime, the system cannot afford the high-fidelity state, triggering a \textbf{Convective Shutdown}~\cite{sato2025convective} and a collapse into the low-energy scoliotic mode.}
