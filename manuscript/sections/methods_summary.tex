...
Metabolic demand for counter-curvature maintenance was modeled as $D(L) \propto L^3$ (elastic strain-energy route: $U \sim EI\kappa^2 L$). Nutrient supply capacity to avascular discs was modeled as $S(L) \propto L^2$ (diffusion-limited by endplate surface area)~\cite{urban2004nutrition}. The relative deficit growth $R(L)/R(L_0)=(L/L_0)$ was mapped to the peak-height-velocity interval using WHO pediatric spine growth charts.

\subsection{Postural Feedback Operating Point}
\label{sec:operating_point}

Where the delayed proprioceptive feedback loop enters the analysis (lumped model and characteristic equation, Supplementary Section~\ref{sec:supp_hopf_rejected}), a single operating point is used throughout this work: $K_P = 120$, $K_D = 12$, resting feedback delay $\tau_0 = 45$\,ms (plant $I = 0.8$, $b = 1.0$, $m = 25$, $L = 0.30$), with the growth-spurt (peak-height-velocity) excursion to $K_D \approx 7$ at $\tau \approx 75$\,ms. The exact Hopf ridge of the rejected delay-Hopf analysis (Supplementary Eq.~\ref{eq:hopf_quadratic_supp}) peaks at $K_D^* \approx 12.4$, $\tau^* \approx 79$\,ms at $K_P = 120$, so the resting point sits inside the stable region and the PHV excursion approaches --- but does not cross --- that boundary.

\subsection{Outcome Metrics}

For each simulation, we computed: (1) \textbf{Equilibrium deviation} $\widehat{D}_{\mathrm{geo}}$ quantifying departure from the gravity-only passive equilibrium configuration, (2) \textbf{Lateral scoliosis index} $S_{\mathrm{lat}} = \max|y|/L_{\mathrm{eff}}$ measuring coronal-plane asymmetry, (3) \textbf{Cobb angle} via linear fits to upper and lower spine segments (standard clinical measure), and (4) \textbf{Lordosis/kyphosis angles} from sagittal curvature profiles, compared against clinical norms (cervical lordosis $40$--$60^{\circ}$, thoracic kyphosis $20$--$45^{\circ}$, lumbar lordosis $40$--$60^{\circ}$)~\cite{white1990clinical}.

\subsection{Clinical Cohort and Bracing Simulation}

We calibrated the model's clinical translation using a simulated cohort of $N=1,000$ pediatric patients (80\% female, ages 10--15, initial Cobb angles $10^\circ$--$30^\circ$, Risser stages 0--4). Patient age and sex were mapped to spinal length $L$ and total body mass $M$ using standard pediatric anthropometric growth charts: spinal length was interpolated using Dimeglio growth data, and body mass was interpolated using WHO weight-for-age charts. For prospective registry validation where direct radiographic spine-length measurement is unavailable, $L$ is approximated as $L = 0.31 \times h_{\mathrm{stand}}$ (Dimeglio scaling from standing height $h_{\mathrm{stand}}$); errors-in-variables correction is applied to $B_g^{\mathrm{eff}}$ estimates to account for proxy uncertainty.
