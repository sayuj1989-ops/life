...
Furthermore, we simulated the thermodynamic cost $P_{\mathrm{counter}}(L)$ across developmental spine lengths $L \in [0.25, 0.55]$~m using the IEC framework. The metabolic power required to maintain the S-curve against passive gravitational sag scales proportionally with $L^2$. In contrast, the proprioceptive supply capacity $S_{\mathrm{proprio}}$ follows a sublinear trajectory. The intersection of these curves defines a critical spine length $L_{\mathrm{crit}} \approx 0.35$~m, marking the precise onset of the Energy Deficit Window (Fig.~\ref{fig:energy_deficit_window}).
