...
Complementing this geometric analysis, we computed the thermodynamic cost of countercurvature, defined as $P_{\mathrm{counter}} \propto L^2 \langle |\kappa_{\mathrm{IEC}} - \kappa_{\mathrm{passive}}|^2 \rangle$. Our simulations reveal a scale-invariant cost ($P_{\mathrm{counter}} \sim L^0$) in the linear regime, indicating that the metabolic demand for maintaining the target curvature remains constant despite increasing length. However, when compared to a proprioceptive supply capacity that scales sublinearly ($S_{\mathrm{proprio}} \sim L^{0.5}$), we identify an ``Energy Deficit Window'' where demand exceeds supply for lengths $L < L_{\mathrm{crit}} \approx 0.35$ m (Fig.~\ref{fig:energy_deficit_window}). % Results verified by scripts/experiment_energy_deficit_window.py
This suggests that while geometric instability threatens the adolescent spine, the primary metabolic bottleneck occurs during early development, necessitating robust proprioceptive gain to bridge the deficit before the supply capacity matures.
