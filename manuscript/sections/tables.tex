...
\begin{table}[h!]
\centering
\caption{The 16 candidate proteins analyzed in this study, grouped by mechanistic role ($\eta_p$ proprioceptive channels, $\eta_a$ active control, $\Gamma_m$ metabolic regulators) and characterized by AlphaFold-derived structural metrics. The anisotropy ratio and morphology classification indicate the relative structural cost of maintaining directional sensing versus volumetric (scalar) function. Genes are prioritized by confidence-weighted scoring from the AFCC pipeline.}
...
Critical Transition ($L_{crit}$) & Spine Length vs. simulated Cobb Angle & $r = 0.983$ \newline Deficit Ratio Max = $38.5$ & $p = 2.74 \times 10^{-22}$ \newline (Model-internal) \\
