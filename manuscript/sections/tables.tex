...
\begin{table}[h!]
\centering
\caption{List of thermodynamic cost proteins ($n=16$) grouped by dissipation term and characterized by AlphaFold structural metrics. The anisotropy ratio and morphology indicate the structural cost of maintaining orientation (vector sensing) vs. volume (scalar maintenance). The free energy dissipation functional is defined in Eq.~\ref{eq:dissipation}. This table lists the 16 key proteins analyzed in the study.}
\label{tab:thermodynamic_cost_proteins}
\scriptsize
\begin{tabular}{llcclllc}
\toprule
\textbf{Gene} & \textbf{UniProt} & \textbf{Dissipation Term ($\eta_p / \eta_a / \Gamma_m$)} & \textbf{Anisotropy} & \textbf{Morphology} & \textbf{pLDDT} & \textbf{Residues} & \textbf{L-Scaling} \\
\midrule
