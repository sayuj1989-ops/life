...
\label{subsec:rod_geometry}

The human spine is modeled as a Cosserat rod with centerline curve $\mathbf{r}(s) \in \mathbb{R}^3$, parameterized by arc length $s \in [0, L]$. To each point $s$, we attach an orthonormal material frame $\{\mathbf{d}_1(s), \mathbf{d}_2(s), \mathbf{d}_3(s)\}$ describing the orientation of the cross-section~\cite{goriely2017mathematics, gazzola2018forward}. The director $\mathbf{d}_3(s)$ aligns with the tangent to the centerline in the absence of shear, while $\mathbf{d}_1(s)$ and $\mathbf{d}_2(s)$ define the principal axes of the vertebral cross-section. The kinematics are governed by the evolution equations:
\begin{align}
...
\label{subsec:dev_info}

We introduce a scalar information field $I(s, t) \in [0, 1]$ representing the coarse-grained expression level of anterior-posterior patterning morphogens (e.g., HOX genes, SHH/BMP gradients). The spatial gradient of this field, $\nabla I$, acts as a "curvature generator," driving the resting geometry of the rod away from the straight configuration. Formally, we define the intrinsic rest curvature $\boldsymbol{\kappa}_{\mathrm{rest}}$ as a function of the information gradient:
\begin{equation}
\boldsymbol{\kappa}_{\mathrm{rest}}(s) = \boldsymbol{\kappa}_{\mathrm{gen}} + \chi_\kappa \nabla I(s),
\label{eq:kappa_rest}
\end{equation}
where $\chi_\kappa$ is a dimensionless geometric coupling constant. This posits that morphogenetic information encodes a target metric for the developing spine, where high rates of change in identity (transition zones) correspond to high intrinsic curvature.

As established in Methods (Eq.~\ref{eq:info_field_spinal}), we utilize a bimodal Gaussian parameterization for $I(s)$, with peaks at the cervical ($s_c=0.80$, amplitude $A_c=0.5$) and lumbar ($s_l=0.25$, amplitude $A_l=0.7$) levels, corresponding to regions of high vertebral identity specification. Physically, these peaks represent zones where the specification of vertebral identity is most robust, driven by the combinatorial expression of HOX genes. The spatial gradient of this field, $\partial I/\partial s$, acts as the "curvature generator" that drives the intrinsic rest curvature away from the straight configuration. Structural analysis using AlphaFold predictions for key HOX proteins (e.g., HOXC8, HOXB13) confirms distinct rigidities that may underpin this spatial specificity~\cite{jumper2021alphafold}.
