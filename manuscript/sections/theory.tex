...
We model the spine as a Cosserat rod defined by a centerline curve $\mathbf{r}(s) \in \mathbb{R}^3$ parameterized by arc length $s \in [0, L]$. To each point $s$, we attach an orthonormal material frame $\{\mathbf{d}_1(s), \mathbf{d}_2(s), \mathbf{d}_3(s)\}$ describing the orientation of the cross-section~\cite{goriely2017mathematics}. The kinematics are governed by the following evolution equations:
\begin{align}
