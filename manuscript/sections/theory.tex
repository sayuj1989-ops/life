...
The proprioceptive control law generating corrective force $F_{\mathrm{control}}$ with feedback delay $\tau$ is:
\begin{equation}
F_{\mathrm{control}}(s,t) = K_P e(s, t-\tau) + K_D \frac{\partial e}{\partial t}(s, t-\tau),
\label{eq:control_law}
\end{equation}
where $K_P$ and $K_D$ are proportional and derivative gains, and the error is $e(s,t) = \kappa_{\mathrm{measured}}(s,t) - \kappa_{\mathrm{target}}$.
...
Assuming solutions of the form $\kappa(s,t) = A e^{\lambda t} y_n(s)$, where $y_n(s)$ are the spatial eigenmodes of the clamped-free column, the characteristic equation determining the stability eigenvalues $\lambda$ is:
\begin{equation}
\rho A \lambda^2 + \eta \lambda + EI \beta_n^4 + (K_P + K_D \lambda) e^{-\lambda \tau} = 0,
\label{eq:dde_characteristic}
\end{equation}
where $\beta_n$ represents the characteristic roots for the spatial boundary conditions.
...
\subsection{Molecular Grounding: The AlphaFold Anisotropy Gap}

While the primary failure mode is governed by macroscopic delay dynamics, we hypothesize that the molecular architecture of the mechanosensory apparatus creates a structural vulnerability. We performed AlphaFold v6 structural analysis of 31 key spinal proteins, classified \emph{a priori} as ``Demand'' (mechanosensors, cytoskeletal force-transducers; $n=19$) or ``Supply'' (metabolic regulators, growth factors; $n=12$), and computed the gyration-tensor anisotropy $\sqrt{\lambda_1/\lambda_3}$ for each predicted structure.

Demand-side proteins exhibited significantly higher structural anisotropy than Supply-side regulators (Mann-Whitney $U$, one-sided $p = 0.002$; Cohen's $d = 1.11$, large effect; Cliff's $\delta = 0.62$, large effect; significant after Bonferroni correction for three tests at $\alpha = 0.0167$). Demand-side proteins had mean anisotropy $2.96 \pm 1.31$ (median 2.80), while Supply-side proteins had mean $1.73 \pm 0.59$ (median 1.62), yielding a \textbf{Demand-Supply Anisotropy Gap} of $70.4\%$.

The most anisotropic proteins---Vimentin (5.57), PTK7 (5.28), Lamin A/C (4.71), ASIC3 (4.56)---are all Demand-side mechanosensors or force-transducers. Conversely, AIS-linked GWAS genes (PAX1, LBX1, BNC2, SOX9) predominantly encode compact transcription factors and metabolic regulators with low anisotropy, consistent with our model: genetic variants alter the \emph{controllers} of the mechanosensory system, while the \emph{instruments} they regulate (high-anisotropy sensors) are the structures most vulnerable to metabolic collapse during the Energy Deficit Window.

We emphasize an important caveat: current AI structural prediction tools cannot directly infer mechanical properties under load~\cite{gomes2022protein, zonta2024sequence}, and confidence scores for large extracellular matrix components are frequently low. Nonetheless, the strong statistical separation ($p = 0.002$) between structurally defined Demand and Supply groups provides a quantitative molecular basis for the vulnerability mechanism.
