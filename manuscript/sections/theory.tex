...
The Human spine is modeled as a one-dimensional Cosserat rod embedded in three-dimensional Euclidean space. However, in the IEC framework, we treat the rod's reference configuration not as a flat segment, but as a manifold whose intrinsic geometry is warped by a developmental information field $I(s)$. This approach allows us to describe the complex, species-specific curvature patterns as arising from an underlying scalar field that modulates the stress-free state of the tissue.

In our biological context, the information field dictates the target curvature $\kappa_{\mathrm{rest}}(s)$, effectively shaping the equilibrium geometry. This shaping is formally described by the Fisher Information metric $\FisherInfo(s)$, which defines the local sensitivity of the target geometry to perturbations in the morphogenetic field:
\begin{equation}
