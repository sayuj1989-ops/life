...
At higher proportional gains, the optimal derivative gain shifted upward and the maximum tolerable delay increased: at $K_p = 200$, optimal $K_d \approx 20$ and $\tau^* \approx 140$\,ms.
...
This computational modeling study introduces the concept of a ``derivative gain gap''---a transient deficit in the rate-sensing component of trunk postural control that arises during rapid adolescent growth. The model suggests that this gap may create a defined instability window through a Hopf bifurcation mechanism, and that the timing and sex-specificity of this window are consistent with the known clinical epidemiology of AIS. A central observation from the model is that the relationship between derivative gain and delay tolerance is non-monotonic, meaning that growth-induced changes may push the system outside a bounded stability corridor even when the proportional gain remains adequate. The simulation results further suggest that individuals with greater intrinsic trunk stiffness (higher $K_p$) may possess a larger stability margin during growth, which could contribute to the clinical heterogeneity of AIS progression. Supplementary analyses suggest that these conclusions are not artifacts of specific parameter choices: the parameter sensitivity analysis (Figure~S1) showed that qualitative predictions held across wide ranges of $\beta$ and $\alpha$, and the Monte Carlo analysis (Figure~S2) was consistent with robustness to individual growth variability.

\subsection{Comparison with Existing AIS Literature}

The derivative gain gap hypothesis is consistent with several previously disconnected lines of evidence in the AIS literature. The documented proprioceptive deficits in AIS patients---including impaired trunk repositioning accuracy and altered vibration perception thresholds \citep{lao2008proprioceptive, simoneau2006altered}---are consistent with the degraded $K_d$ predicted by our model and may represent the clinical correlate of the derivative gain gap. The well-established correlation between PHV and AIS onset \citep{sanders2017maturity, burwell2009aetiopathogenesis} is consistent with the model's prediction that the vulnerability window is temporally linked to peak growth velocity. Increased postural sway in AIS patients \citep{dalleau2012postural} is consistent with the reduced stability margin during the derivative gain gap.

The model's predicted 2.8-fold higher female instability probability is broadly consistent with the reported 3:1 female-to-male ratio for curves exceeding 30 degrees \citep{cheng2015prevalence}, although this correspondence depends on assumed differential $\beta$ values. The model's prediction that spontaneous stabilization follows growth deceleration is consistent with the clinical observation that curve progression in AIS typically ceases at skeletal maturity \citep{weinstein2008natural}.

The model is also consistent with biomechanical progression theories: the Hueter-Volkmann effect (asymmetric loading driving asymmetric growth) \citep{stokes2006hueter, villemure2009growth} may serve as a downstream progression mechanism once the initial deviation is established by the control-theoretic instability proposed here. The simulated brace augmentation result---in which adding an external $K_{d,\text{ext}}$ restored stability---is consistent with the clinical observation that brace efficacy depends on compliance duration rather than corrective force magnitude \citep{weinstein2013bracing, negrini2018sosort}, and that nighttime braces providing primarily haptic feedback show clinical efficacy \citep{price1997efficacy}.

The stochastic sensitivity results suggest that individuals with noisier proprioceptive signals---potentially due to genetic variants affecting mechanosensory channel function \citep{assaraf2020piezo2, coste2010piezo}---may have a lower effective stability threshold and therefore greater susceptibility to AIS onset during the growth spurt. These correspondences provide supportive context but do not constitute validation; each requires independent prospective testing.
