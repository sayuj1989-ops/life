...
First, this is a purely computational modeling study. No patient data were collected or analyzed, and the model has not been validated against clinical measurements. The results should therefore be interpreted as hypothesis-generating rather than confirmatory.

Second, the model is restricted to a single sagittal plane. Clinical AIS is a three-dimensional deformity involving coupled lateral bending, axial rotation, and sagittal plane changes. The single-degree-of-freedom model captures the fundamental instability mechanism but cannot reproduce the full complexity of the clinical deformity.

Third, the growth velocity curves were modeled as simplified Gaussian pulses based on population-level CDC/WHO reference data. Individual growth patterns exhibit considerable variability, and the use of population means may obscure important inter-individual differences.

Fourth, the degradation function for $K_d$ (Eq.~\ref{eq:kd_degrade}) was postulated based on plausibility rather than directly measured. The specific parameter values ($\beta$, $\alpha$) were chosen to produce physiologically reasonable behavior but require empirical validation.

Fifth, the model does not account for neuromuscular adaptation. In reality, the central nervous system continuously recalibrates its control gains, and some individuals may adapt sufficiently quickly to avoid the instability window entirely. The rate of recalibration is likely an important determinant of AIS susceptibility but was not explicitly modeled.

Sixth, the inverted pendulum model uses a rigid-body approximation. The spine is a multi-segmental, viscoelastic structure, and the assumption of a single lumbar pivot simplifies the dynamics considerably.

Seventh, while we report sex-specific timing differences, the differential $\beta$ values used for males and females were assumed rather than derived from independent evidence. Further work is needed to determine whether sex differences in proprioceptive maturation are sufficient to account for the observed female preponderance.
