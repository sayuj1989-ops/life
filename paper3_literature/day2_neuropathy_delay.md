...
- **Total Delay**: Normal adult $\tau$ is often modeled around 150-170 ms. Age-related neuropathic changes can push $\tau$ past 200 ms. ~~In the Paper 2 model (adolescent growth spurt), a $\tau > 200$ ms was a critical threshold for instability when combined with rapid geometrical changes.~~ **[SUPERSEDED 2026-09-04]** That Paper-2 threshold claim does not survive re-derivation: the exact Hopf boundary of the committed model (see `~/life/scripts/hopf_exact.py`; ridge peak $\tau_c = 79$ ms at $K_d^* = 12.4$, $K_p = 120$) puts every cited instability threshold an order of magnitude lower, and the delay-Hopf onset story itself is rejected (Supplementary S10.2). The ageing-deadline argument must be rebuilt on the exact boundary before any of these numbers are reused. In ageing, this increased delay acts concurrently with degraded controller gains (due to central and sensory changes) and reduced actuator output (sarcopenia).
