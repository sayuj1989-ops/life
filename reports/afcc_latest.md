...
- Parameters: pLDDT threshold >= 70 for geometry; Smoothing window = default
- Notes: 9 structures analyzed. Source config: research/alphafold_countercurvature/config/targets.yaml
# AFCC Daily Summary: 2026-01-19

## Overview
- **Candidates Processed:** 9
- **Metrics File:** `metrics.csv`
- **FBN1** was excluded (not found in AFDB).

## Top Anisotropic Candidates
...
## Key Insights
- **LMNA and PIEZO2** maintain their status as top high-anisotropy candidates ("Tension Rods"), confirming the Slack-Rod hypothesis potential.
- **COL1A1** shows high anisotropy but low confidence (pLDDT 52.7), suggesting significant disorder or flexible regions which may complicate rod modeling.
- **FBN1** remains absent from AFDB, requiring alternative structural sources or homology modeling if it is to be retained in the pipeline.
