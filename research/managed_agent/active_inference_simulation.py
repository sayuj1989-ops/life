...
TAU         = 0.03           # proprioceptive delay [s]  (30 ms, ensures stability boundary is well-posed)
...
PI_Y0_PRIOR = 250.0          # prior precision on position error [N m rad^-1] (Must be > mgL ~ 176 for L=0.45)
PI_Y1_PRIOR = 100.0          # prior precision on velocity error [N m s rad^-1]
PI_Y2_PRIOR = 8.0            # prior precision on integral error
PI_V0_PRIOR = 200.0          # prior precision on dynamic PE

# ---- Belief-update learning rates -------------------------------------------
KAPPA_MU    = 0.5            # belief update rate (gradient descent on F)
KAPPA_MU2   = 0.5            # rate for second generalised coordinate

# ---- Precision dynamics parameters ------------------------------------------
KAPPA_PI    = 0.30           # precision update rate  [s^-1]
GAMMA_VAR   = 0.80           # running-variance low-pass rate  [s^-1]
ALPHA_PREC  = 5e5            # sensitivity of precision to PE variance
