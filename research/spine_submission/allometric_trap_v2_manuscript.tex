...
Adolescent Idiopathic Scoliosis (AIS) affects 2--4\% of adolescents
worldwide, yet the mechanism linking the pubertal growth spurt to spinal
instability has remained unexplained for over a century. We present the
\textbf{Allometric Trap} hypothesis: a unified biomechanical framework
demonstrating that AIS arises from a transient \emph{precision collapse}
in the proprioceptive postural control loop during rapid longitudinal growth.

We model the adolescent trunk as a delayed inverted pendulum governed by an
Active Inference agent performing gradient descent on variational free energy.
Under linear Gaussian assumptions this formulation recovers PID control with gains
determined by dynamic sensory precisions \citep{Baltieri2019, FristonFEP2019}.
During the growth spurt, trunk centre-of-mass height $L(t)$ increases faster
than the agent's internal model can update, generating systematic velocity
...
delayed system \citep{Insperger2011, Insperger2013}. This constitutes a
\textbf{Hopf bifurcation}: the system transitions from damped to growing
oscillation, and the spine buckles laterally.

We validate this framework against four independent evidence streams:
(i)~a hardened cross-species allometric power law ($\alpha = -0.259 \pm 0.053$,
$R^{2} = 0.72$, $n=12$ species spanning four orders of body-mass magnitude);
(ii)~sex-stratified AIS epidemiology in 11 longitudinal cohorts demonstrating
that growth rate velocity predicts curve progression with $\DAIC = 45.8$ over
static height alone and $r(\dot{L},\,\text{risk}) = 0.73$ ($p = 0.017$);
(iii)~a structural protein anisotropy analysis of $n=31$ AIS-relevant proteins
(AlphaFold v4) confirming a demand--supply gap [Mann-Whitney $p = 0.002$,
Cohen's $d = 1.11$, Cliff's $\delta = 0.62$, Bonferroni-corrected]; and
(iv)~a computational Active Inference simulation reproducing bifurcation dynamics
with physiological parameters (Python 3.11, seed 42, $\Delta t = 5$\,ms).

The framework yields three falsifiable predictions. The highest-priority
testable prediction is NAD$^{+}$ rescue: NMN supplementation (500\,mg/kg/d)
during peak growth velocity in heterozygous proprioceptor-stressed mice
(Pv-Cre $\times$ Runx3$^{fl/+}$; \citealt{Blecher2017}) should prevent curvature
in $\geq70\%$ of mice that would otherwise develop curves $>10^{\circ}$, at
an 18-month cost of $\sim\$500$K (NIH R21/NIAMS). This work reframes AIS from
a structural defect to a \textbf{control-system failure} precipitated by the
allometric mismatch between gravitational demand ($\sim L^{4}$) and metabolic
supply ($\sim L^{2}$) during the transient adolescent growth window.

\textbf{Word count (abstract):} $\sim$310\\[0.5em]

\noindent\textbf{Significance Statement:} Scoliosis affects millions of children
with no known cause and no curative treatment. We show mathematically,
computationally, and epidemiologically that scoliosis is a control-system failure
precisely timed to the adolescent growth spurt: the spine grows faster than the
nervous system can recalibrate, collapsing the proprioceptive feedback gain below
a critical threshold and initiating spinal buckling. This reframing unites
previously disconnected observations---proprioceptive deficits, female
predominance, growth-rate dependence, and cross-species patterns---into a single
predictive equation, and proposes a concrete, immediately fundable experimental
test.
...
$\tau \leq 100$\,ms. The full stability chart (Insperger-Stepan semi-discretization
\citealt{Insperger2011}) was used to verify the small-delay approximation across
the parameter range $(L \in [0.40, 0.75]\,\text{m},\,\tau \in [20, 100]\,\text{ms})$;
the closed-form expression Eq.~\eqref{eq:Kdcrit} is accurate to within 8\% at
$\tau = 100$\,ms, $L = 0.75$\,m.
