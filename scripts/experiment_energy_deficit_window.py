...
def bimodal_gaussian(s_norm, Ac=0.5, sc=0.80, sigmac=0.08, Al=0.7, sl=0.25, sigmal=0.10, I0=0.3):
    bump_c = Ac * np.exp(-((s_norm - sc)**2) / (2 * sigmac**2))
    bump_l = Al * np.exp(-((s_norm - sl)**2) / (2 * sigmal**2))
    return bump_c + bump_l + I0

def compute_gradient(field, s_norm):
    return np.gradient(field, s_norm)
...
def main():
    ...
    for L in L_range:
        ...
        # Information field
        I_field = bimodal_gaussian(s_norm)

        # Gradient with respect to normalized coordinate
        # This ensures that curvature does not drop inversely with L.
        # So we use s_norm to get constant maximum curvature across lengths.
        grad_I = compute_gradient(I_field, s_norm)

        # Target curvature from IEC-1 coupling
        kappa_target = chi_kappa * grad_I

        # Assume constant stiffness and no active moment
        E_field = np.full_like(s, E0)
        M_active = np.zeros_like(s)

        # External distributed load is weight per unit length
        distributed_load = rho * A_cross * g  # 10.791 N/m
    ...
    # Closest L to L0
    idx0 = (df['L'] - L0).abs().argmin()
    S0 = df.loc[idx0, 'P_counter']
    ...
    # We only care about intersections where P_counter > S_proprio_alpha05
    # Since they intersect at L=0.35, we look for L > 0.35
    df_post_intersect = df[df['L'] > 0.35]
    if len(df_post_intersect) > 0:
        L_crit = 0.35  # They are equal at 0.35 by definition
        plt.axvline(L_crit, color='k', linestyle='--', alpha=0.5, label=f'$L_{{crit}} \\approx {L_crit:.2f}$ m')

        # Shade the energy deficit window
        deficit_mask = df['L'] >= L_crit
        plt.fill_between(df.loc[deficit_mask, 'L'],
                         df.loc[deficit_mask, 'S_proprio_alpha05'],
                         df.loc[deficit_mask, 'P_counter'],
