...
def generate_markdown_report(filepath, results, total_time):
    ...
    with open(filepath, 'w') as f:
        ...
        f.write("This experiment maps 'virtual protein' profiles to macroscopic spinal mechanics using PyElastica.\n")
        f.write("It tests the **Vector-Scalar Mismatch** hypothesis: that spinal stability requires a balance between "
        ...
        f.write("| Profile | Anisotropy (Vector) | Curvature Drive (Scalar) | S_lat (Scoliosis Index) | Cobb Angle (deg) | Max Torsion |\n")
        f.write("|---|---|---|---|---|---|\n")
        ...
        for r in results:
            ...
            f.write(f"| {name} | {aniso:.2f} | {curv:.2f} | {s_lat:.4f}{flag} | {cobb:.2f} | {tor:.4f} |\n")

        f.write("\n## Interpretation\n")
        f.write("1. **WildType Control**: Shows baseline stability.\n")
        f.write("2. **Marfan-Like**: Low anisotropy leads to reduced stiffness against perturbation.\n")
        f.write("3. **Piezo Gain**: High active curvature drive can induce over-correction.\n")
        f.write("4. **Scoliotic Risk**: The combination of low anisotropy and high drive maximizes deformation (High S_lat).\n")
