...
from src.spinalmodes.countercurvature.multi_segment import solve_multi_segment_cosserat_buckling
...
def main():
    ...
    # Adding Polygenic Threshold Analysis to the dataset
    baseline_margin = 5.3
    polygenic_combined = -26.3
    threshold_data = pd.DataFrame({
        'Scenario': ['Baseline', 'Combined Variants'],
        'Stability_Margin_ms': [baseline_margin, polygenic_combined],
        'Clinical_Prevalence_pct': ['N/A', '2-4%']
    })

    threshold_data.to_csv('outputs/experiments/polygenic_threshold_analysis.csv', index=False)
